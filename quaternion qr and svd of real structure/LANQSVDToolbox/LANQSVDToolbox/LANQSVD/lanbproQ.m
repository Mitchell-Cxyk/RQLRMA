function [U,B_k,V,p,ierr,work] = lanbproQ(varargin)

%LANBPROQ Lanczos bidiagonalization with partial reorthogonalization.
%   LANBPROQ computes the Lanczos bidiagonalization of a quaternion
%   matrix using the  with partial reorthogonalization.
%
%   [U_k,B_k,V_k,R,ierr,work] = LANBPROQ(A,K,R0,OPTIONS,U_old,B_old,V_old)
%
...
if isnumeric(A) || isstruct(A)
    ...
    Atrans = varargin{2};
    if ~isstr(Atrans) && ~isa(Atrans,'function_handle')
    ...
    k=varargin{5};
    if narg < 6, p = rand(m,4)-0.5; else p=varargin{6}; end
...
cgs = 0;             % Flag for switching between iterated MGS (0) and CGS (1).
...
npu = 0; npv = 0; ierr = 0;
% p = p(:); % p is a quaternion vector  p=[p0,p2,p1,p3]
% Prepare for Lanczos iteration.
if isempty(U)
    V = zeros(n,4*k); U = zeros(m,4*k);
    ...
    j = size(U,2)/4; % Size of existing factorization
    % Allocate space for Lanczos vectors
%     U = [U, zeros(m,4*(k-j))];
%     V = [V, zeros(n,4*(k-j))];
    U=extendQcol(U,zeros(m,4*(k-j))); 
    V=extendQcol(V,zeros(n,4*(k-j)));
...
%     anorm=normQ(A,2);
    ...
    int = [1:j]';
    % quaternion reorthogonalization
    [p,beta(j+1),rr] = reorthQ(U,p,beta(j+1),int,gamma,cgs);
    npu =  rr*j;  nreorthu = 1;  force_reorth= 1;
    
    % Compute Gerscgorin bound on ||B_k||_2
    if est_anorm
        anorm = FUDGE*sqrt(normQ(timesQ(transQ(B_k),B_k),1));
    end
...
if LANBPRO_TRUTH==1
    MUTRUE = zeros(k,4*k); 
    NUTRUE = zeros(k-1,4*(k-1));%NUTRUE = zeros(k-1,k-1);
    MU = zeros(k,k);
    NU = zeros(k-1,k-1);
    
    MUTRUE_AFTER = zeros(k,4*k); NUTRUE_AFTER = zeros(k-1,4*(k-1));
    MU_AFTER = zeros(k,k); NU_AFTER = zeros(k-1,k-1);
end
% U=U
% V=V
% Perform Lanczos bidiagonalization with partial reorthogonalization.
[rU,cU]=size(U);cU=cU/4;
[rV,cV]=size(V);cV=cV/4;
for j=j0:k
%     j=j
    if waitb
    ...
    if beta(j) ~= 0
        U(:,[j,cU+j,2*cU+j,3*cU+j]) = p/beta(j);
    else
        U(:,[j,cU+j,2*cU+j,3*cU+j]) = p;
    end
    ...
    if j==1
        if isnumeric(A)
            r = timesQ(At,U(:,[1,cU+1,2*cU+1,3*cU+1]));
        elseif isstruct(A)
            r = A.R\U(:,1);
        else
            r = feval(Atrans,U(:,1));
        ...
        if isnumeric(A)
            r = timesQ(At,U(:,[j,cU+j,2*cU+j,3*cU+j])) - beta(j)*V(:,[j-1,cV+j-1,2*cV+j-1,3*cV+j-1]);
        elseif isstruct(A)
        ...
        if alpha(j)<gamma*beta(j) && elr && ~fro
            ...
            stop = 0;
            while ~stop
                t = timesQ(transQ(V(:,[j-1,cV+j-1,2*cV+j-1,3*cV+j-1])),r);
                r = r - timesQ(V(:,[j-1,cV+j-1,2*cV+j-1,3*cV+j-1]),t);
                alpha(j) = normQ(r);
                if beta(j) ~= 0
                    beta(j) = beta(j) + t(1,1);
                end
        ...
        if j>1 && LANBPRO_TRUTH
            ...
            NUTRUE(1:j-1,[j-1,(k-1)+j-1,2*(k-1)+j-1,3*(k-1)+j-1]) = timesQ(transQ(V(:,[1:(j-1),cV+1:(j-1),2*cV+1:(j-1),3*cV+1:(j-1)])),r)/alpha(j);
        end
        ...
        if onesided~=-1 && ( fro || numax(j) > delta || force_reorth ) && alpha(j)~=0
            ...
            % Reorthogonalize v_j
            [r,alpha(j),rr] = reorthQ(V,r,alpha(j),int,gamma,cgs);
    ...
    if alpha(j) < max(n,m)*anorm*eps && j<k,
        ...
        bailout = 1;
        for attempt=1:3
            r = rand(m,4)-[0.5*ones(m,1),zeros(m,3)];
            if isnumeric(A)
                r = timesQ(At,r);
            elseif isstruct(A)
                r = A.R\r;
            ...
            nrm=timesQ(transQ(r),r);
            nrm=sqrt(nrm(1,1)); % not necessary to compute the norm accurately here.
            int = [1:j-1]';
            [r,nrmnew,rr] = reorthQ(V,r,nrm,int,gamma,cgs);
...
%         NUTRUE_AFTER(1:j-1,j-1) = V(:,1:j-1)'*r/alpha(j);
        NUTRUE_AFTER(1:j-1,[j-1,(k-1)+j-1,2*(k-1)+j-1,3*(k-1)+j-1]) = timesQ(transQ(V(:,[1:(j-1),cV+1:(j-1),2*cV+1:(j-1),3*cV+1:(j-1)])),r)/alpha(j);
    end
    
    
    if alpha(j) ~= 0
        V(:,[j,cV+j,2*cV+j,3*cV+j]) = r/alpha(j);
    else
        V(:,[j,cV+j,2*cV+j,3*cV+j]) = r;
    end
    ...
    if isnumeric(A)
        p = timesQ(A,V(:,[j,cV+j,2*cV+j,3*cV+j])) - alpha(j)*U(:,[j,cU+j,2*cU+j,3*cU+j]);
    elseif isstruct(A)
    ...
    if beta(j+1)<gamma*alpha(j) && elr && ~fro
        ...
        stop = 0;
        while ~stop
            t = timesQ(transQ(U(:,[j,cU+j,2*cU+j,3*cU+j])),p);
            p = p - timesQ(U(:,[j,cU+j,2*cU+j,3*cU+j]),t);
            beta(j+1) = normQ(p);
            if alpha(j) ~= 0
                alpha(j) = alpha(j) + t(1,1);
            end
...
%         MUTRUE(1:j,j) = U(:,1:j)'*p/beta(j+1);
        MUTRUE(1:j,[j,k+j,2*k+j,3*k+j]) = timesQ(transQ(U(:,[1:j,cU+1:j,2*cU+1:j,3*cU+1:j])),p)/beta(j+1);
    end
    ...
    if beta(j+1) < max(m,n)*anorm*eps  && j<k,
        ...
        for attempt=1:3
            ...
            nrm=timesQ(transQ(p),p);
            nrm=sqrt(nrm(1,1)); % not necessary to compute the norm accurately here.
            int = [1:j]';
            [p,nrmnew,rr] = reorthQ(U,p,nrm,int,gamma,cgs);
            npu = npu + rr*length(int(:));  nreorthu = nreorthu + 1;
            mu(int) = m2*eps;
            if nrmnew > 0
                % A vector numerically orthogonal to span(Q_k(:,1:j)) was found.
    ...
    if LANBPRO_TRUTH==1
        ...
        MUTRUE_AFTER(1:j,[j,k+j,2*k+j,3*k+j]) = timesQ(transQ(U(:,[1:j,k+1:j,2*k+1:j,3*k+1:j])),p)/beta(j+1);
    end
end
if waitb
    close(waitbarh)
...
if nargout==1
    U = B_k;
elseif k~=size(U,2)/4 || k~=size(V,2)/4
